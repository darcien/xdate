...
---------------------------------------------------------------------------------*/
  ...
  each(methodSubjects, function(subject, fieldIndex) {
    ...
    if (fieldIndex != DAY) {
      ...
      if (fieldIndex != YEAR) {
        ...
        proto['add' + (subjectPlurals[fieldIndex] || subject)] = function(
          delta,
          preventOverflow,
        ) {
        ...
        proto['diff' + (subjectPlurals[fieldIndex] || subject)] = function(
          otherDate,
        ) {
  ...
  function _add(xdate, fieldIndex, delta, preventOverflow) {
    ...
    if (intDelta != delta && fieldIndex < MILLISECONDS) {
      _add(
        xdate,
        fieldIndex + 1,
        (delta - intDelta) * unitsWithin[fieldIndex],
        preventOverflow,
      );
    }
  }
  ...
  function _diff(xdate1, xdate2, fieldIndex) {
    ...
    if (fieldIndex == FULLYEAR || fieldIndex == MONTH) {
      ...
      v =
        Math.round((clear2 - clear1) / DAY_MS) +
        (xdate2 - clear2 - (xdate1 - clear1)) / DAY_MS;
    } else {
      v =
        (xdate2 - xdate1) /
        [
      ...
      if (m[13]) {
        ...
        if (m[14]) {
          ...
          d.setUTCMinutes(
            d.getUTCMinutes() +
              (m[15] == '-' ? 1 : -1) *
                (Number(m[16]) * 60 + (m[18] ? Number(m[18]) : 0)),
          );
        }
      } else {
...
---------------------------------------------------------------------------------*/
  ...
  proto.toUTCString = proto.toGMTString = function(
    formatString,
    settings,
    uniqueness,
  ) {
  ...
  function processTokenString(
    ...
    var out = '';
    while (end > 0) {
      replacement = getTokenReplacement(
        xdate,
        tokenString.substr(0, end),
        getField,
        getSetting,
        useUTC,
      );
  ...
  function _getDesignator(getField, getSetting) {
    return getField(HOURS) < 12
      ? getSetting('amDesignator')
      : getSetting('pmDesignator');
  }
...
---------------------------------------------------------------------------------*/
  ...
  function _setField(d, useUTC, fieldIndex, args) {
    d['set' + (useUTC ? 'UTC' : '') + methodSubjects[fieldIndex]].apply(
      d,
      args,
    );
  }
